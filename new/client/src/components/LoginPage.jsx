...
const LoginPage = ({setFlag}) => {
  ...
  // GSAP Entry Animations & Interactive Floating Micro-interactions
  useGSAP(() => {
    const tl = gsap.timeline({ defaults: { ease: 'power4.out' } });

    // 1. Smooth Fade-in for your custom background
    tl.fromTo(
      '.bg-image-layer',
    ...
    // 2. High-impact drop-in for RADETAPE text with a slight scale bounce
    tl.fromTo(
      brandRef.current,
    ...
    // 3. Luxurious split-reveal for the Glassmorphic login box
    tl.fromTo(
      cardRef.current,
      { y: 60, opacity: 0, backdropFilter: 'blur(0px)' },
      { y: 0, opacity: 1, backdropFilter: 'blur(24px)', duration: 1.2 },
      '-=0.9'
    );

    // 4. Staggered element presentation (Labels, inputs, buttons)
    tl.fromTo(
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(to bottom, rgba(19, 11, 8, 0.25), rgba(48, 30, 24, 0.85)), url('/LGB-2.jpg')`
        }}
      />
      ...
      <main className="relative z-10 flex flex-col items-center justify-center flex-grow w-full max-w-md px-4 my-6">
        ...
        <h1
          ...
          style={{
            ...
            background: 'linear-gradient(270deg, #6e4730, #b8835c, #ebd2b3, #6e4730)',
        ...
        >
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
            ...
            {/* Email */}
            <div ref={addToRefs} className="flex flex-col space-y-1.5">
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl bg-black/20 border border-white/10 focus:border-[#b0784c]/60 focus:bg-black/40 text-white placeholder-stone-600 focus:outline-none transition-all duration-300 text-sm"
                required
              />
            </div>

            {/* Password */}
            <div ref={addToRefs} className="flex flex-col space-y-1.5 relative">
              ...
              <div className="flex justify-between items-center">
                ...
                <a
                  href="#forgot"
                  className="text-[11px] text-[#b0784c] hover:text-white tracking-wide transition-colors"
                >
                  Forgot?
                </a>
              </div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full px-4 py-3 rounded-xl bg-black/20 border border-white/10 focus:border-[#b0784c]/60 focus:bg-black/40 text-white placeholder-stone-600 focus:outline-none transition-all duration-300 text-sm pr-10"
                  required
                />
          ...
          {/* Divider */}
          <div ref={addToRefs} className="relative flex py-4 items-center">
      ...
      <footer className="relative z-10 w-full max-w-7xl mx-auto px-8 py-6 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 text-[10px] text-stone-500 tracking-widest uppercase font-bold border-t border-white/5">
        <div className="flex items-center space-x-6">
          <span className="opacity-40">OFFICIAL PARTNER:</span>
